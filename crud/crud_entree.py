...
def ajouter_entree(entree):
    ...
    sql = """
        INSERT INTO gestion_entree (id_fournisseurs, date_commande, statut_commande, date_reception, observation)
        VALUES (%s, %s, %s, %s, %s)
    """

    values = (
        entree.id_fournisseurs,
        entree.date_commande,
...
def consulter_entree(id_entrees, id_fournisseurs):
    ...
    table = PrettyTable([
        "ID", "Identifiant", "Fournisseur", "Commande", "Statut", "Reception",
    ...
    sql = """
        SELECT e.id_entrees AS ID,
               e.id_fournisseurs AS identifiant,
               f.matricule AS fournisseur,
               e.date_commande AS commande,
               e.statut_commande AS statut,
               e.date_reception AS reception,
               e.observation,
               le.id_produits AS ID_produit,
               p.ref_produit AS référence,
               p.nom_commercial AS nom,
               le.numero_lot AS lot,
               le.peremption AS Exp,
               le.quantite AS Qte,
...
def lister_entree():
    ...
    table = PrettyTable(["ID", "Fournisseur", "Date commande", "Statut"])

    sql = """
        SELECT id_entrees, id_fournisseurs, date_commande, statut_commande
        FROM gestion_entree
